...
// Demo roles: 'admin', 'examiner', 'lecturer', 'student', or null
...
function RolePicker() {
  ...
  const onChange = (r: string) => {
    ...
    // reload to apply role changes in demo
    if (typeof window !== 'undefined') {
...
function RoleLinks() {
  ...
  const can = (r: string) => role === 'admin' || role === r;

  return (
    <>
      {can('admin') && (
        <Link href="/admin/dashboard" className="text-sm text-foreground hover:text-primary transition">Admin</Link>
      )}
      {can('examiner') && (
        <Link href="/examiner/dashboard" className="text-sm text-foreground hover:text-primary transition">Examiner</Link>
      )}
      {can('lecturer') && (
        <Link href="/lecturer/dashboard" className="text-sm text-foreground hover:text-primary transition">Lecturer</Link>
      )}
    </>
  );
}